...
<methodCall>
    ...
    <params>
        ...
        public async Task<List<VillageData>> GetVillagesAsync()
        {
            ...
            try
            {
                ...
                Console.WriteLine($"✅ Получено деревень: {villages.Count}");
                
                // Показываем примеры
                for (int i = 0; i < Math.Min(5, villages.Count); i++)
                {
                    var v = villages[i];
                    Console.WriteLine($"  [{v.Id}] {v.Name} ({v.X}, {v.Y})");
                }
        ...
        public async Task<bool> SendToProxyServerAsync(List<VillageData> villages)
        {
            Console.WriteLine($"📤 Отправка {villages.Count} деревень в прокси-сервер...");
            ...
            using (var client = new HttpClient())
            {
                foreach (var village in villages)
                {
                    try
                    {
                        ...
                        var response = await client.PostAsync("http://localhost:5000/api/village", content);
                    ...
                    {
                        Console.WriteLine($"Ошибка отправки деревни {village.Id}: {ex.Message}");
                    }
                }
            }
            
            Console.WriteLine($"✅ Отправлено успешно: {successCount}/{villages.Count}");
    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            Console.WriteLine("АВТОМАТИЧЕСКИЙ КЛИЕНТ ИГРОВОГО СЕРВЕРА");
            Console.WriteLine("="*60);
            Console.WriteLine();
            
            // Читаем credentials из .env
            string email = Environment.GetEnvironmentVariable("GAME_EMAIL") ?? "kara.bridges1991@comejoinuspro.org";
            string password = Environment.GetEnvironmentVariable("GAME_PASSWORD") ?? "0307koolKL123";
            ...
            try
            {
                ...
                // Шаг 3: Получение деревень
                var villages = await client.GetVillagesAsync();
                
                if (villages.Count == 0)
                {
                    Console.WriteLine("❌ Деревни не получены");
                    return;
                }
                
                // Шаг 4: Отправка в прокси-сервер
                await client.SendToProxyServerAsync(villages);
                
                Console.WriteLine();
                Console.WriteLine("🎉 ГОТОВО!");
                Console.WriteLine($"Получено и отправлено {villages.Count} деревень");
                Console.WriteLine();
                Console.WriteLine("Теперь можно использовать Discord бота:");
                Console.WriteLine($"!time capx4 {villages[0].Id}[1] {(villages.Count > 1 ? villages[1].Id : villages[0].Id)}");
            }
            catch (Exception ex)
            {
